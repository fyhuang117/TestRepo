...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabls">
  ...
  <tr>
    <td align="left">上级类型</td>
    <td><select name="par"  class="form-control">
    	<option value='0'>无</option>
    	<?php 
    		$select_partType_list = $_partType->getList();
    		foreach ($select_partType_list as $_partType_){
				if($_partType->id != $_partType_->id){
					if($_partType->par == $_partType_->id){
						echo '<option value="'.$_partType_->id.'" selected>'.$_partType_->nam.'</option>';
					}else{
						echo '<option value="'.$_partType_->id.'" >'.$_partType_->nam.'</option>';
					}
				}
    		}
    	?>
    ...
    <td>
    	<?php 
    		...
    		foreach ($partBand_list as $_partBand_){
				if(strstr($_partType->ban,','.$_partBand_->id.',')){
					echo '<input type="checkbox" name="band_id[]" value="'.$_partBand_->id.'" checked/>'.$_partBand_->nam;
				}else{
